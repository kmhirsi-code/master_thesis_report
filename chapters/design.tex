...
\begin{enumerate}
    ...
    \item \textbf{Model-assisted labeling.} Once the manually verified corpus was sufficiently large, a preliminary model was trained and used to auto-label the remainder of the dataset. Segments with a model confidence of 95\% or above were accepted automatically, covering roughly two-thirds of the remaining unlabeled recordings.
...
A further design option explored for the GBT path is \textbf{asymmetric soft labels}, intended to address the fact that human annotators naturally place boundaries with some imprecision, and that the physiological consequences of timing errors are asymmetric. For the Make Onset, detecting slightly \emph{early} is acceptable (the finger is already moving), whereas a \emph{late} detection directly increases perceived input latency. Conversely, for the Break Onset, a slightly \emph{late} detection is tolerable (the click has already been registered), whereas an \emph{early} detection prematurely resets the input state. The soft label generator uses a split-normal Gaussian centred on each onset frame: a wide left standard deviation and narrow right standard deviation for Make, and narrow left and wide right for Break, encoding directional uncertainty directly into the training target rather than penalising the model equally for both error directions. This objective is implemented and supported by the training pipeline but was not used for the GBT results reported in this thesis, which are trained on hard labels with the focal loss objective described above.
