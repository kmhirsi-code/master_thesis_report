...
Once click-intent boundaries are extracted, the system searches for the hardware threshold pair (Actuation Point, Rapid Trigger sensitivity) that best reproduces those boundaries in firmware.

\subsection{Firmware State Machine Simulation}
The optimizer embeds a frame-accurate simulation of the two firmware trigger modes. The \textbf{simple threshold} mode is a 2-state machine: a press is registered when the analog depth signal crosses the make threshold upward, and released when it crosses the break threshold downward. The \textbf{Rapid Trigger} mode is a 3-state machine (Idle $\to$ Pressed $\to$ RT-Released) that tracks rolling peaks and valleys: once pressed, the button releases when depth drops by more than the RT sensitivity below the last peak; from the released state, it re-actuates when depth rises by more than the sensitivity above the last valley, and fully resets to Idle only when the signal returns to zero. Both modes mirror the exact transition rules in the device firmware, validated against hardware-logged event timestamps. The full state transition diagrams for both modes are given in Figures~\ref{fig:fsm_nort} and~\ref{fig:fsm_rt} in Chapter~\ref{chap:implementation}.

\subsection{Search Space and Objective}
The search space is two-dimensional: make threshold and break threshold, each drawn from 20 discrete depth levels (0--120 in steps of 6, matching the firmware's integer representation). CMA-ES is chosen over grid search for two reasons. First, the optimal make and break thresholds are correlated---a higher actuation point typically permits a proportionally larger RT sensitivity---so a method that adapts the full covariance matrix between the two parameters converges significantly faster than diagonal-step methods. Second, the firmware FSM is cheap to evaluate (microseconds per recording), so the $O(n^2)$ CMA-ES convergence budget is a practical constraint on search iterations rather than compute time.

Each candidate threshold pair is scored by a unified scalar loss:
\[
\begin{aligned}
  \mathcal{L} ={}& \bar{d}_{\text{latency}} + w_{\text{miss}} \cdot N_{\text{miss}} + w_{\text{fp}} \cdot N_{\text{fp}} \\
  & {}+ w_{\text{early}} \cdot (N_{\text{make\_err}} + N_{\text{break\_err}})
\end{aligned}
\]
where $\bar{d}_{\text{latency}}$ is the average temporal distance between each simulated trigger and its matched ground-truth intent boundary, $N_{\text{miss}}$ counts undetected real clicks, $N_{\text{fp}}$ counts phantom clicks, and $N_{\text{make\_err}} + N_{\text{break\_err}}$ penalises timing errors at boundaries. The penalty weights $(w_{\text{miss}}, w_{\text{fp}}, w_{\text{early}})$ are configurable and allow the user to express preferences (e.g.\ prioritise recall over precision for a competitive FPS player).

\subsection{Pareto Frontier and Output}
The optimizer is run independently for each candidate feature combination (different preprocessing signals can serve as the make or break detection input). After convergence, a Pareto frontier over \{error rate, average latency\} is computed and returned to the front-end client for visualization. The user inspects the trade-off curve and selects a Pareto-optimal configuration, rather than receiving a single black-box recommendation. A complementary 40$\times$40 grid sweep is run after CMA-ES to provide a uniform landscape view of the full threshold space for diagnostic purposes.
