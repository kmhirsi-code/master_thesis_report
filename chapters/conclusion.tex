...
The heuristic bootstrap's deployment F1 varies from 0.1\,\% to 75.0\,\% across
individual participants (CV~0.79 on the left channel), and this spread does not
track cleanly with any collected demographic or self-reported gaming attribute.
The heuristic applies one fixed geometric rule (peak prominence and derivative
thresholds tuned on aggregate statistics) uniformly across the population; when
individual grip pressure, resting depth, or micro-tremor magnitude differs enough
from that aggregate profile, the rule fails outright rather than degrading
gracefully. This is precisely the failure mode motivating a learned, per-user-adaptive
classifier over a fixed geometric one.
