...
hyperparameter set due to the cost of CPU-only training (\textasciitilde45 minutes per
run). No systematic search over architecture depth, dropout rate, learning rate
schedule, or number of dilation stages was performed. The observed LOPO collapse
($0.597 \pm 0.174$) may therefore reflect a poorly tuned model rather than a
fundamental architectural limitation. A GPU-accelerated Optuna sweep over the FCN's
hyperparameter space, combined with a larger right-button corpus, is the most likely
path to closing the performance gap with the Random Forest.
...
signatures cluster differently at the start versus end of a 40-minute session.
A classifier that ingests global session features --- session elapsed time, running
mean of the resting drift channel, recent inter-click interval statistics --- would
be better equipped to distinguish intent from noise as fatigue accumulates.
