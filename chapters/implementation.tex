...
Training uses a focal cross-entropy variant that suppresses the gradient contribution of easy background frames, reducing the dominance of the majority class. Early stopping is evaluated on hard-label accuracy over the held-out validation partition. When the soft-label path is enabled, a custom gradient and Hessian objective replaces the standard multiclass cross-entropy to align with the continuous target distribution, but this configuration was not exercised for the results in this thesis.
