...
The parameter optimizer is implemented as a standalone Rust service that communicates with the front-end client via named-pipe IPC. It receives the preprocessed analog depth signal and the ground-truth click regions from the ML manager, runs the CMA-ES sweep, and returns the Pareto frontier and full landscape grid as a JSON sidecar.

The core of the service is a frame-accurate simulation of the two hardware trigger modes, illustrated in Figures~\ref{fig:fsm_nort} and~\ref{fig:fsm_rt}. In the \textbf{simple threshold} mode (Figure~\ref{fig:fsm_nort}), actuation is a two-state machine: the button transitions to the pressed state when analog depth exceeds the make threshold and returns to idle when it drops back below. In \textbf{Rapid Trigger} mode (Figure~\ref{fig:fsm_rt}), the FSM is extended to five states. Once pressed, the button releases as soon as depth drops by the RT sensitivity below the last observed peak. From the RT-released state the button can re-actuate when depth rises above the last trough by the RT sensitivity, or fully reset to idle when depth returns to zero. These transition rules mirror the device firmware exactly, including the absence of a haptic blind window in the current hardware revision.
