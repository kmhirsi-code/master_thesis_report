...
\subsection{Deep Temporal Models: FCN and DETR-style Architectures}
...
To transform these continuous feature maps into discrete temporal boundaries (such as click onsets or releases), transformer-based architectures modelled after the DEtection TRansformer (DETR)~\cite{carion2020detr} can be adapted for 1D sequences. A DETR-style decoder utilizes multi-head cross-attention to match a fixed set of learned "event queries" against the FCN's sequence features. This frames sequence labeling as a direct set prediction problem, allowing for the precise localisation of temporal boundaries and bypassing heuristic post-processing tools like non-maximum suppression.
