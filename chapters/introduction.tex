...
\paragraph{A hybrid labeling pipeline.} For analog HID click intent, deriving
...
and then improving boundary quality through model-assisted iteration.

\paragraph{A four-class temporal sequence classifier.} With two model paths: a shallow
path (Random Forest, Gradient Boosted Trees) operating on hand-crafted
sliding-window features for fast per-session retraining (a few minutes on a
consumer CPU, depending on channel and corpus size), and a deep path
(dilated residual FCN) operating on raw telemetry with a learned receptive field. This classifier is then used to segment click telemetry into labeled sequences for downstream analysis.

\paragraph{A firmware-faithful threshold optimizer.} Exhaustively evaluating the
...
comparative candidate points), giving users a way to inspect why a suggested AP/RT point
is proposed; this interpretability goal was not itself the subject of a dedicated
usability evaluation in this thesis.
